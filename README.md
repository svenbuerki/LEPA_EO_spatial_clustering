...
**Figure 5.** Five-panel network figure, one panel per bottleneck lineage. Panels are **ordered by total habitat area, with within-BL connectivity as the secondary tie-break** (largest at top-left, smallest at bottom-right): the top row (BL4, BL5, BL3) shows the three largest BLs, and the bottom row (BL1, BL2) shows the two smallest. Area is the primary *Ne* proxy (carrying capacity → drift floor); connectivity is the secondary stratification for BLs of similar size. This ordering supports predictions of S-allele diversity: larger, more connected BLs (top row) are expected to retain richer allele complements because both census-size and gene-flow buffer drift, whereas the smallest BLs (bottom row) are predicted to be most eroded and therefore the highest-priority targets for restoration interventions. **Within each panel, locations are placed geographically**: x = longitude, y = latitude (from the Peggy lat/lon source CSV), so north is up and east is right and the relative positions of populations within a BL reflect their true cardinal arrangement. To make every BL fill its panel regardless of geographic extent, the lon/lat ranges are normalised independently per axis to [0, 1]; the true real-world extent is given as a **"≈ X.X km E–W × Y.Y km N–S" annotation** placed in whichever corner of each panel is farthest from data and edges (chosen automatically per BL so the label does not collide with nodes or connection lines). Solid edges indicate connected pairs (≤500 m) and dashed edges indicate near-miss pairs (500 m–2 km). Node fill encodes drift index on the blue (DI = 0, weak drift) → white → red (DI = 1, strong drift) gradient matching Figure 2. Node size encodes total population size; node shape (diamond = connected, circle = isolated) distinguishes locations with at least one neighbour within 500 m from fully isolated locations. The largest populations carry the same EO code / location ID / census size labels as in Figure 1, so individual locations can be tracked across figures. Strip header colors are matched to the BL shading in Figure 2 and the hull colours in Figure 3.

**Per-BL standalone slide variants** are also produced by the script as `figures/EO_BL_drift_panel_{BL1..BL5}_presentation.{png,pdf}` (gitignored — see `.gitignore`). Each per-BL variant carries the same data as the corresponding facet of Figure 5 but is formatted for single-slide use: explicit **"km east" / "km north" axes** with tick marks (anchored at the SW corner of the BL's bounding box) so the reader can read distances directly off the figure, and a **within-BL group-pairwise distance heatmap** dropped into whichever corner is farthest from the data. The inset heatmap uses the same lower-triangle YlOrRd recipe as the inter-BL distance heatmap in Figure 4 (in-cell km values, no separate colour legend), so the per-lineage drift slides and the between-lineage fragmentation slides read with a consistent visual grammar.
